...
// Retry fetch with 5s delay for 429/5xx (up to 10 retries), with optional SSE logging
let _sseWriter = null;
function setSSEWriter(writer) { _sseWriter = writer; }
...
async function fetchWithRetry(url, opts = {}, retries = 10) {
  for (let i = 0; i <= retries; i++) {
    ...
    if ((res.status === 429 || res.status >= 500) && i < retries) {
      if (_sseWriter) _sseWriter.write('log', { log: `Rate limited (${res.status}), retrying in 5s... (${i + 1}/${retries})`, error: true });
...
// Page title prefixes that should be skipped (templates, user pages, etc.)
const SKIP_PAGE_PREFIXES = ['template:', 'user:', 'talk:', 'file:', 'mediawiki:', 'module:', 'category:', 'help:', 'draft:'];

async function runBatchImport({ apiBase, wikiName, categories, projectName, store, broadcastFn, sessionCode, sseWriter, signal, resume }) {
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    try {
      ...
      // Filter out template/system pages
      const contentPages = pages.filter(p => !SKIP_PAGE_PREFIXES.some(prefix => p.toLowerCase().startsWith(prefix)));
      const unique = contentPages.filter(p => !parsedPages.has(p));
      categoryPages.set(catName, unique);
      for (const p of contentPages) parsedPages.add(p);
    ...
    if (i < categories.length - 1) await delay(200, signal).catch(() => {});
  }

  stats.pagesTotal = [...categoryPages.values()].reduce((sum, p) => sum + p.length, 0);
  parsedPages.clear(); // Reset — we'll track during parsing

  // Create or reuse existing project (for resume)
  try {
    store.createProject(projectName, { source: 'wiki-batch' });
    // Save batch params into project for future resume
    const projData = store.getProject(projectName);
    projData.batchMeta = { apiBase, wikiName, categories };
    store._save(projectName, projData);
  } catch (e) {
    if (resume) {
      // Resuming — collect already-imported page titles to skip
      try {
        const projectData = store.getProject(projectName);
        for (const cat of (projectData.categories || [])) {
          for (const chunk of (cat.chunks || [])) {
            if (chunk.metadata?.page_title) parsedPages.add(chunk.metadata.page_title);
          }
        }
        sseWriter.write('log', { log: `Resuming: found ${parsedPages.size} already imported pages, skipping them` });
      } catch (_) {}
    } else {
  ...
  // Broadcast project creation immediately so sidebar shows it
  ...
  for (const catName of categories) {
    ...
    // Create category
    try {
      store.createCategory(projectName, catName, { source: 'wiki-batch', skipHistory: true });
    } catch (e) {
    ...
    for (const pageTitle of pages) {
      ...
      try {
        ...
        // Save chunks immediately per page
        const flatChunks = textChunks.map(chunk => ({
          id: chunk.id,
          text: chunk.text,
        ...
        try {
          store.bulkAddChunks(projectName, catName, flatChunks, { source: 'wiki-batch', skipHistory: true });
          // Broadcast so sidebar updates in real-time
          if (broadcastFn && sessionCode) {
            broadcastFn(sessionCode, 'data:changed', { project: projectName });
          }
        } catch (saveErr) {
          sseWriter.write('log', { log: `Error saving "${pageTitle}": ${saveErr.message}`, error: true });
        }
  ...
  if (stats.chunksCreated > 0) {
    store.recordHistory(projectName, 'wikiBatchImport', 'Built ' + stats.chunksCreated + ' chunks from batch wiki import', 'wiki-batch');
  }
