...
const RENDERED_METADATA_VERSION = 2;
...
async function prepareLocalWiki(cacheRoot, url, options = {}) {
  ...
  const manifest = {
    ...
    categoriesCount: 0,
    lastError: null,
    lastSyncAt: forceRefresh ? null : existing?.lastSyncAt || null,
    ...detected,
  };
  await saveManifest(cacheRoot, manifest);
  return { wiki: manifest, ready: false, cached: !!existing && forceRefresh };
}
...
async function ingestLocalWiki(cacheRoot, url, sseWriter, signal, options = {}) {
  const forceRefresh = !!options.forceRefresh;
  const prepared = await prepareLocalWiki(cacheRoot, url, options);
  const manifest = prepared.wiki;
  const paths = getCachePaths(cacheRoot, manifest.wikiId);

  if (prepared.ready && !forceRefresh) {
  ...
  try {
    ...
    if (forceRefresh) {
      sseWriter.write('status', { phase: 'refresh', log: 'Refreshing local wiki cache...' });
      await removeIfExists(paths.pages);
      await removeIfExists(paths.categories);
      await removeIfExists(paths.renderedDir);
      if (manifest.acquisition.kind === 'wikimedia-dump') {
        await removeIfExists(paths.dumpFile);
      }
    }
    if (manifest.acquisition.kind === 'wikimedia-dump') {
    ...
    manifest.updatedAt = new Date().toISOString();
    manifest.lastSyncAt = manifest.updatedAt;
    await saveManifest(cacheRoot, manifest);
    return manifest;
  } catch (error) {
...
async function loadRenderedMetadata(paths, title) {
  try {
    ...
    if (data?.version !== RENDERED_METADATA_VERSION) return null;
    return data;
  } catch {
    return null;
  }
      ...
      if (result?.html) {
        ...
        if (Object.keys(extracted.infobox || {}).length || extracted.text) {
          return {
            ...
            version: RENDERED_METADATA_VERSION,
...
async function runLocalDatasetBuild({ cacheRoot, wikiId, categories, projectName, store, broadcastFn, sessionCode, sseWriter, signal, resume }) {
  ...
  for (const category of projectData.categories || []) {
    for (const chunk of category.chunks || []) {
      ...
      if (chunk.id) existingChunkIds.add(String(chunk.id).trim());
    }
  }
  ...
  const addChunksToProject = (categoryName, chunks) => {
    ...
    for (const chunk of chunks) {
      ...
      if (!id) continue;
      if (existingChunkIds.has(id)) {
        stats.skipReasons.duplicateChunkId++;
        continue;
      }
  ...
  for await (const line of rl) {
    ...
    if (selectedLookup.size && !normalizedPageCategories.some(category => selectedLookup.has(category.toLowerCase()))) {
      stats.skipped++;
      stats.skipReasons.category++;
      stats.pagesDone++;
      continue;
    }
    if (existingPageTitles.has(page.title)) {
      stats.skipped++;
      stats.skipReasons.duplicatePage++;
      stats.pagesDone++;
      continue;
    }
  ...
  projectData.localWikiMeta = {
    ...
    lastSyncAt: manifest.lastSyncAt || manifest.updatedAt || null,
